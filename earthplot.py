...
def xyticks(ax, xticks=None, yticks=None, xticklabels=None, yticklabels=None,
            ...
            xminor=None, yminor=None, xtick_params=None, ytick_params=None, **kwargs):
    ...
    xticks_kw, yticks_kw = {}, {}
    xticks_dict = {'xticks':'ticks', 'xticklabels':'labels', 'xha':'ha'}  # ha doesn't work
    yticks_dict = {'yticks':'ticks', 'yticklabels':'labels', 'yha':'ha'}
...
def vector(ax, vx, vy, x=None, y=None, width=None, headwidth=3.,
    ...
    flux = ax.quiver(x, y, vx, vy, pivot='mid', width=width, headwidth=headwidth, 
                     ec=ec, fc=fc, lw=lw, scale=scale, units=units, zorder=zorder, transform=ccrs.PlateCarree())
    return flux
...
def heatmap(ax, data, xlabels, ylabels, vmin=None, vmax=None, gridcolor='k', gridlinewidth=1, 
            spines_visible=True, valfmt="{x:.1f}", text_threshold=None, textkw={},
            **kwargs):
    """
    ...
    **kwargs
    ...
    ax.set_xticks(np.arange(data.shape[1]), labels=xlabels)  # , ha="right"
    ...
    ax.spines[:].set_visible(spines_visible)

    ax.set_xticks(np.arange(data.shape[1]+1)-.5, minor=True)
    ax.set_yticks(np.arange(data.shape[0]+1)-.5, minor=True)
    ax.grid(which="minor", color=gridcolor, linestyle='-', linewidth=gridlinewidth)
    ax.tick_params(which="minor", bottom=False, left=False)

    _ = annotate_heatmap(im, valfmt=valfmt, threshold=text_threshold, **textkw)
    ...
    **kwargs
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            kw.update(color=textcolors[im.norm(abs(data[i, j])) > threshold])
